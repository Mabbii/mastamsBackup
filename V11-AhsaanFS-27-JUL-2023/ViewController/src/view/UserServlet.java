...
public class UserServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
                   ...
                   PreparedStatement statement = conn.prepareStatement("SELECT USER_PROFILE_ID, USER_PROFILE_IMAGE FROM USER_PROFILE WHERE USER_PROFILE_ID=?");
                   statement.setInt(1, new Integer(Integer.parseInt(imgeId)));
                   ResultSet rs = statement.executeQuery();
                   if(rs.next()){
                       Blob blob = rs.getBlob("USER_PROFILE_IMAGE");
